<script lang="ts">
	...
	$: canCreate =
		!moneyError && players.length >= 3 && players.every((p) => p.name.trim().length > 0);
	...
	{:else}
		<section class="create-game">
			...
			{#each players as player, idx}
				<div class="player">
					<Textfield kind="name" bind:value={player.name} label="Name" required />
			...
			{/each}
			{#if players.length < 6}
				<Button class="material-icons" on:click={handleAddPlayer}>Add Player</Button>
			{/if}
			{#if players.length < 3}
				<div>Minimum 3 players are required</div>
			{/if}
			<div>
				<Button variant="raised" disabled={!canCreate} on:click={handleCretateGame}>
					Create Game
				</Button>
			</div>
		</section>
